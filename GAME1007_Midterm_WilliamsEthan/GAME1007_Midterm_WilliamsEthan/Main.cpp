...
int main()
{
	...
	bool gameOver = false; // DONE Define a bool called gameOver, initialized to false
	short currPlayer; /* Keeps track of current player and is also used as a simple index for the players array. */

	srand((unsigned)time(NULL));  // DONE!! Call srand() with a parameter time(NULL) cast to an unsigned integer (as you've done before)

	// DONE If a call to BuildAbilities() with a parameter abilityArray is equivalent to 1	
	...
	BuildPlayer(players[0], abilityArray); // DONE Call BuildPlayer() with two parameters: players[0] and abilityArray

	players[1].SetName("Player 2");				   // Done Call the SetName() member function of players[1] with a parameter "Player 2" similar to line 129

	BuildPlayer(players[1], abilityArray); // DONE Call BuildPlayer() with two parameters: players[1] and abilityArray

	currPlayer = 1 + rand() % 2; 	// DONE Set currPlayer to be rand() % 2

	cout << '\n' << players[currPlayer].GetName() << " wins the coin toss and goes first!" << endl;

	while (gameOver == false) // DONE While gameOver is equivalent to false
	{
		// DONE Output that it is the current player's turn, calling the GetName() member function of the current player just like line 134
		players[currPlayer].GetName();
		cout << "It is  " << currPlayer << " turn";
		// DONE Call PrintAbilities with two parameters: players[currPlayer].GetAbilities() and ABIL_PLR
		...
		if (gameOver == false)  //DONE  If gameOver is equivalent to false
		{
			currPlayer = (currPlayer == 0 ? 1 : 0);	// DONE Set currPlayer to the opposite player (try to use conditional operator here, but however you like is fine)
		}
		// DONE Output that the game is over and the current player wins, calling its GetName() member function like lines 134
		cout << "The game is over and " << players[currPlayer].GetName() << "wins";
		system("pause");
