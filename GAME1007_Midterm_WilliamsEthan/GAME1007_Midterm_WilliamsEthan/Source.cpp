...
int main()
{
	...
	// Define a bool called gameOver, initialized to false
	short currPlayer; /* Keeps track of current player and is also used as a simple index for the players array. */

	// Call srand() with a parameter time(NULL) cast to an unsigned integer (as you've done before)
	// If a call to BuildAbilities() with a parameter abilityArray is equivalent to 1
		// Return 1
	players[0].SetName("Player 1"); /* I left this for you to show how we call a member function of an object. */
	// Call BuildPlayer() with two parameters: players[0] and abilityArray
	// Call the SetName() member function of players[1] with a parameter "Player 2" similar to line 129
	// Call BuildPlayer() with two parameters: players[1] and abilityArray
	// Set currPlayer to be rand() % 2
	cout << '\n' << players[currPlayer].GetName() << " wins the coin toss and goes first!" << endl;
	// While gameOver is equivalent to false
	{
		// Output that it is the current player's turn, calling the GetName() member function of the current player just like line 134
		// Call PrintAbilities with two parameters: players[currPlayer].GetAbilities() and ABIL_PLR
		...
		// If gameOver is equivalent to false
			// Set currPlayer to the opposite player (try to use conditional operator here, but however you like is fine)
	}
	// Output that the game is over and the current player wins, calling its GetName() member function like lines 134
	system("pause");
	// Return 0
}
